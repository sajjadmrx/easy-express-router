...
export abstract class EasyRouter {
  ...
  static initControllers(options?: InitControllerOptions): Router {
    if (!_controllers.length) throw new Error('controllers list has empty');

    if (options) {
      if (options.bodyParser) easyExpress.use(bodyParser.json());
    }

    _controllers.forEach((controller: any) => {
      const prefix: string = Reflect.getMetadata(
        RouteMetaKeys.PREFIX,
        controller
      );
      const prefixOptions: PrefixRouteOptions = Reflect.getMetadata(
        RouteMetaKeys.PREFIX_OPTIONS,
        controller
      );

      const routes: MetaRoute[] =
        Reflect.getMetadata(MetaKeys.routes, controller) || [];
      ...
      routes.forEach((route: MetaRoute) => {
        ...
        if (route.middlewares.length || prefixMiddlewares.length) {
          const middlewares: Middleware[] = [
            ...new Set([...prefixMiddlewares, ...route.middlewares])
          ];

          (easyExpress as any)[route.method](
            `${prefix}/${path}`,
            middlewares,
            route.handler
          );
